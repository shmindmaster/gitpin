...
| --- | --- | --- |
| `npx` fails on Node | Node older than 20 | After 0.6.4 npm publication is verified, install Node 20+ and retry `npx -y gitpin@0.6.4 init --client codex` |
...
1. After 0.6.4 npm publication is verified, run `npx -y gitpin@0.6.4 doctor` with `GITPIN_REGISTRY` set.
