...
| --- | --- | --- |
| `npx` fails on Node | Node older than 20 | Install Node 20+ and retry `npx -y gitpin@latest init --client codex` |
...
1. Run `npx -y gitpin@latest doctor` with `GITPIN_REGISTRY` set.
