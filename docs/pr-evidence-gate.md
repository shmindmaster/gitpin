...
```yaml
  written-controls:
    ...
    steps:
      ...
      # CrewScore v0.6.12 release commit; use a full SHA, not a movable tag.
      - uses: shmindmaster/crewscore@bc5c331431afea0b2f66dac3e1b05864e91ecdb8
        with:
          scan-path: .
...
This optional example commit-pins the CrewScore `v0.6.12` release; choose and verify the exact release commit appropriate for your repository. CrewScore observes written text only. It does not prove runtime enforcement, agent obedience, certification, or compliance. Pin its Action commit and ruleset, and keep `.crewscore.yml` on the trusted base branch.
