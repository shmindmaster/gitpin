...
describe('GitHub Action trust boundary', () => {
  ...
  it('commit-pins the repository self-gate to the published 0.6.4 Action source', () => {
    const workflow = readFileSync(resolve('.github/workflows/evidence-gate.yml'), 'utf8');
    expect(workflow).toContain('commit-pin the self-gate to the published v0.6.4 release source');
    expect(workflow).toContain('uses: shmindmaster/gitpin@514799cb0eae55e965e51d510d5e7a0a7604c9cc');
