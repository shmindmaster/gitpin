...
import { z } from 'zod';
import { getRepoStatus, getRepoCommits, getRepoFile, searchRepoCode, compareRepoCommits, getRepoManifest, getRepoRecentChanges, getRepoTests } from './git';
...
function createServer(): McpServer {
  ...
  server.registerTool('wiki.catalog', {
    ...
    if (view === 'stale') return json(catalog.filter((r: any) => r.stale));
    if (view === 'sync') return json(catalog.map(({ name, status, docCount }: any) => ({ name, status, docCount })));
    return json(catalog);
  });
  ...
  server.registerTool('wiki.analyze', {
    ...
    inputSchema: z.object({ operation: z.enum(['gaps', 'compare']), repositories: z.array(z.string()).max(20).optional() }),
  ...
  server.registerTool('repo.inspect', {
    ...
    inputSchema: z.object({ repository: z.string(), operation: z.enum(['status', 'commits', 'manifest', 'tests', 'changes']), limit: z.number().int().min(1).max(50).optional() }),
  ...
  server.registerTool('repo.read', {
    ...
    inputSchema: z.object({ repository: z.string(), sourcePath: z.string(), lineStart: z.number().int().positive().optional(), lineEnd: z.number().int().positive().optional() }),
  }, async ({ repository, sourcePath, lineStart, lineEnd }) => json(await getRepoFile(repository, sourcePath, lineStart, lineEnd)));
  ...
  server.registerTool('repo.compare', {
    ...
    inputSchema: z.object({ repository: z.string(), base: z.string().min(7).max(40), head: z.string().min(7).max(40) }),
